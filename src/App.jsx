...
import { formSchema, orderTypes, transportTypes, vendorOptions } from "./lib/validators";
    ...
    <main>
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          {parsedCircuit && <div className="badge">Layer inferred: {parsedCircuit.layer}</div>}
        </FormSection>
          ...
          {values.transportType === "UPE" && (
            <div className="grid grid-2">
              <FormSectionField label="UPE Name" error={errors.upeName}>
                <input value={values.upeName} onChange={(event) => handleChange("upeName", event.target.value)} />
              </FormSectionField>
            </div>
          )}
        </FormSection>
          ...
          <div className="grid grid-2">
            ...
            {values.transportType === "UPE" && (
              <FormSectionField label="UPE Vendor" error={errors.upeVendor}>
                <select value={values.upeVendor} onChange={(event) => handleChange("upeVendor", event.target.value)}>
                  {vendorOptions.map((vendor) => (
                    <option key={vendor} value={vendor}>
                      {vendor}
                    </option>
        ...
        <FormSection title="E) Service Parameters">
          <div className="grid grid-2">
            <FormSectionField label="VLAN ID" error={errors.vlanId}>
              <input value={values.vlanId} onChange={(event) => handleChange("vlanId", event.target.value)} />
            </FormSectionField>
